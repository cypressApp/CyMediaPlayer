...
fun QrCodeGeneratorComposable(
    ...
    val qrcodeState by remoteReceiverViewModel.qrCodeInfoState

    remoteReceiverViewModel.start()
    remoteReceiverViewModel.getQrCodeInfo()
//    app.le("IP:${networkUtilRepository.getWifiIpAddress()}")
    ...
    Scaffold { paddingValues ->
        Column(
            ...
            Text(text = if(remoteReceiverState.isClientVerified){ "Verified" }
                        else if(remoteReceiverState.isClientConnected){ "Connected" }
                        else { "" })
            Spacer(modifier = Modifier.height(16.dp))
            Image(
                bitmap = generateQRCode(qrcodeState).asImageBitmap(),
